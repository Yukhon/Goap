tool
extends Node
class_name GOAPActionPlanner, "res://assets/Item/16x16 RPG Item Pack/Item__00.png"

# Declare member variables here. Examples:
# var a: int = 2
# var b: String = "text"

var state_atoms = [] # list of state atoms 
var actions = []
...
class State:
	...
	func check(condition):
		return (condition.mask & mask) == condition.mask and  (value & condition.mask) == condition.value # check if the state fits the condition
...
class AStarNode: # this node is used for A star tree 
	var state: State
	var previous: int
	var last_action
	var cost: float
...
#	print("during parse, input state is")
#	print(string)
	var value = 0
	var mask = 0
	var regex = RegEx.new()
...
#		print(rv)
		...
		if v:
			value |= rv
	var output = State.new(value, mask)
#	print("state is")
#	print(output.value)
	return output
